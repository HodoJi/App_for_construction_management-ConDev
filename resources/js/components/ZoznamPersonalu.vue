            ...
            <div class="row">
                <div class="col-12">
                    <div class="mb-3">
                        ...
                        <input class="form-control form-control-lg" id="inputName" type="text" @keyup="findUsers"
                               v-model="employeeName" placeholder="" aria-label=".form-control-lg example">
                ...
                <div class="col-lg-3 col-md-6 col-sm-12" v-for="employee in searchedWorkers.users" :key="employee.id">
                    <div class="mb-3">
                        <div class="card "
                             :class="{'bg-warning bg-opacity-75' : employee.cID == this.$route.params.id, 'bg-danger bg-opacity-50' : employee.cID == null}">
                            <div class="card-body">
                                ...
                                <p class="card-text small" v-if="employee.cID !== null"><span class="fw-bold">{{ employee.name }}</span>
                                    už je priradený/á k stavenisku <span class="fw-bold">{{employee.title}}</span>
                                    (#{{employee.cID}})</p>
                                <p class="card-text small" v-if="employee.cID == null"><span class="fw-bold">{{ employee.name }}</span>
                                    ešte nie je priradený/á k žiadnemu stavenisku!</p>
                            </div>
                            <div class="card-footer bg-transparent">
                                <button type="button" class="btn btn-success float-end"
                                        @click="addUserToConstruction(employee.id)"><i class="fas fa-check"></i> Pridať
                                    zamestnanca ku stavenisku
                                </button>
                            </div>
                        </div>
                    </div>
        ...
        methods: {
                            ...
                            this.$axios.get(this.$BASE_PATH + `api/construction-workers-show/${this.$route.params.id}`)
                                .then(response =>
                                {
                                    ...
                                    if (response.data)
                                    {
                                        ...
                                        {
                                            Swal.fire({
                                                title: "Zoznam robotníkov",
                                                html: "Chyba:<br>" + "Žiadny personál nie je na stavenisku.",
                                                icon: "warning"
                                            });
                                        }
                                    }
            ...
            addUserToConstruction(id)
            {
                if (id > 0)
                {
                    this.$axios.get(this.$BASE_PATH + 'sanctum/csrf-cookie').then(() =>
                    {
                        this.$axios.put(this.$BASE_PATH + `api/construction-assign-employee`, {
                            id: id,
                            constrId: this.$route.params.id
                        })
                            .then(response =>
                            {
                                if (response.data)
                            ...
                            .then(response =>
                            {
                                if (response.data)
                                {
                                    ...
                                    if (!this.searchedWorkers.success)
                                    {
                                        Swal.fire({
                                            title: "Personál",
                                            html: "Chyba:<br>" + this.searchedWorkers.message,
                                            icon: "warning"
                                        });
                                    }
                                }
